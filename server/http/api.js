...
// 看板聚合查询集中在 server/http/dashboard-views.js，v1 与 v2 共用同一份，
// 避免"同一块看板两个端数字不一致"。
app.get('/api/dashboard/workers', requireAuth, (req, res) => {
  res.json(dashViews.dashboardWorkers(db));
});

app.get('/api/dashboard/cs', requireAuth, (req, res) => {
  res.json(dashViews.dashboardCs(db));
});

app.get('/api/dashboard/customers', requireAuth, (req, res) => {
  res.json(dashViews.dashboardCustomers(db));
});

app.get('/api/dashboard/trends', requireAuth, (req, res) => {
  res.json(dashViews.dashboardTrends(db, req.query.days));
});
